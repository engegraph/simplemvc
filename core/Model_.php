...
class Model extends \Illuminate\Database\Eloquent\Model
{
    ...
    public function push(array $Relations = [])
    {
        ...
        try
        {
            Manager::connection($Conn)->beginTransaction();
            $Push($this, $Data);
            $result = $this->save();
            Manager::connection($Conn)->commit();
            return $result;
        }
        catch (\Exception $e){
            Manager::connection($Conn)->rollBack();
    ...
    public function __set($key, $value)
    {
        ...
        if(key_exists($key, $this->columns) && !in_array($key,[self::CREATED_AT, self::UPDATED_AT]))
